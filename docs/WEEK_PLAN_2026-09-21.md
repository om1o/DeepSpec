...
Separately, ordinary cloud sync currently has no reconnect retry listener; only offline AI estimates have a reconnect upgrade watcher. Correct any unsupported automatic-retry promise. A future persistent upload queue must capture account ownership, survive reload, send current versions, invalidate stale completions and report pending/failed state. Do not replay every old record over newer cloud work or call a partial queue production-ready.
