...
Durable cloud-save acknowledgements are now implemented and verified with focused tests and a controlled browser upload failure/reload. See the build log and artifacts/qa/2026-09-20T19-57-10-269Z/report.md. History displays unconfirmed/failed records after reload and distinguishes inspection-only acknowledgements. Local edits invalidate prior receipts. Remaining next increment: refresh receipts while History stays open and provide an explicit per-record retry there, with account guards, visible failure and no automatic bulk replay. Server version conflict handling and an automatic reconnect queue remain separate prerequisites before unattended multi-device uploads.

The live History refresh and explicit per-record cloud-save action are now implemented. Duplicate clicks are blocked while pending; failures retain device evidence. Retry preserves a newer inspection already loaded from cloud and stops if it cannot retain that evidence locally. Controlled failure -> real retry -> live acknowledgement -> reload passed in the browser. This supersedes the earlier refresh/retry next-step note. Before automatic reconnect uploads: define server revision/conflict behavior, keep pending operations account-bound, deduplicate detail writes, and test timeout requests that continue server-side. Do not turn this manual retry into bulk unattended replay without those gates.

Inspection rollout now has a concrete command: `npm run verify:supabase -- --inspection`. It checks schema before fixture writes and, once available, tests inspection save/read/update, retained human/AI evidence and cross-account write denial. The current live run is blocked by the missing column; the ordinary cloud verifier passed with checked fixture cleanup. See PART_INSPECTION_PILOT.md and the build log. Apply no automatic queue or pilot cloud-persistence claim until this inspection gate passes after deployment. Public QA credentials cannot remove the temporary anonymous auth accounts.

A further stale-data path was found and removed: login/session restoration previously bulk-uploaded every older local record without server version checks. Login now leaves those bytes intact and directs explicit retries to Saved scans. New scans still save automatically. Auth tests and before/after browser QA verify the change; the final report is artifacts/qa/2026-09-20T20-35-50-148Z/report.md. Automatic old-record replay must remain disabled until server revisions and conflicts are handled. This reduces an overwrite risk while preserving manual recovery; it does not create multi-device conflict resolution.
