...
- `npm run eval:identify:public` now defines the 300-sample public-launch gate with a balanced Hugging Face tree sample and verifier minimum of 300 attempted samples. A live Hugging Face tree check returned 300 sample paths. This is not a substitute for a completed live provider run.
...
| P1-003 | High | Result accuracy | The fixture alternator no longer escalates in the QA fixture, but safety calibration still needs broader proof. | `TEST_ENGINE_IDENTIFICATION` uses `safetyTriage: "can_help"` and `isSafetyCritical: false`; browser QA showed `Useful match`. | Expand eval metrics to track safety false positives so ordinary electrical parts are not escalated unless visible risk exists. |
...
| P1-015 | High | Eval | Beta release eval is fixed at 50 cases, and a separate public-launch command now enforces a 300-sample gate. A completed live 300-sample provider run is still required before public launch. | `npm run eval:identify:release` passed 50 fixed samples; `npm run eval:identify:public` runs `--sample-set public --sample-size 300` and verifies `--min-sample-size 300`; `eval-identify.test.mjs` covers balanced 150 damage / 150 parts selection. GitHub issue: #90. | Run the 300-sample public eval with provider quota available and track accuracy, invalid response rate, latency, safety false-positive rate. |
