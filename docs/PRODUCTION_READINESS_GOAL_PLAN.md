...
| P1-003 | High | Result accuracy | The fixture alternator was treated as a safety-check case. | Live QA result showed high confidence alternator plus safety-critical warning. | Recalibrate safety triage so ordinary electrical parts are not escalated unless visible risk exists. |
...
| P1-008 | High | Cloud UI | UI can say cloud sync is ready even when end-to-end verification fails. | Early Access showed cloud ready; verifier failed anonymous sign-in. | Add runtime cloud health state: configured, auth-ok, storage-ok, parent row upsert, durable dataset detail writes, RLS-ok, last verified. |
