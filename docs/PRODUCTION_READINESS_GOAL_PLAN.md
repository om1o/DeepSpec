...
- `npm run eval:identify:public` now defines the 300-sample public-launch gate with a balanced Hugging Face tree sample, verifier minimum of 300 attempted samples, and summary metrics for accuracy, invalid responses, provider failures, retries, latency, and rule-derived safety false positives. A live Hugging Face tree check returned 300 sample paths. This is not a substitute for a completed live provider run.
...
| P1-003 | High | Result accuracy | The fixture alternator no longer escalates in the QA fixture, and eval summaries now count rule-derived safety false positives. Broader calibration still needs a completed public eval. | `TEST_ENGINE_IDENTIFICATION` uses `safetyTriage: "can_help"` and `isSafetyCritical: false`; browser QA showed `Useful match`; `buildEvalSummary()` reports `safetyEscalationCount`, `safetyFalsePositiveCount`, and `safetyFalsePositiveRate`. | Run the 300-sample public eval with provider quota available and use the safety false-positive metric to tune escalation. |
