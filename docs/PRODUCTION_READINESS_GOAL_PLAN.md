...
| P1-002 | High | Result UX | Result output is closer to a visual result sheet, but it is still not fully Google Lens-like. | Result now has candidates, image evidence, ranked sources, ask/refine actions, uncertainty, one `Safety state` card, and Match/Evidence/Sources/Ask tabs inside the image-first two-pane result sheet. | Browser-test the tabbed sheet on mobile and desktop, then keep tightening toward a true Lens bottom sheet/carousel flow after provider and cloud gates are stable. |
