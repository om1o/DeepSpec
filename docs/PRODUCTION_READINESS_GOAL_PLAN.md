...
- Browser QA on May 22, 2026 passed the remaining scanner mobile fallback coverage: `/scan?test=1` at `390x844` rendered `Test scan ready` and `Test engine photo` without camera-denied copy; `/scan` with denied camera showed `Camera access needed` plus `Upload photo` and `Paste image`.
- Browser upload of `public/test-fixtures/engine-scan-test.jpg` reached a saved result screen, but the live identify request returned `500` because the local server at `127.0.0.1:5192` is missing `GEMINI_API_KEY`.
...
- `npm run eval:identify:public` now defines the 300-sample public-launch gate with a balanced Hugging Face tree sample, verifier minimum of 300 attempted samples, and required summary metrics for accuracy, invalid responses, provider failures, retries, latency, OCR usage, and rule-derived safety false positives/false negatives. A live Hugging Face tree check returned 300 sample paths. This is not a substitute for a completed live provider run.
...
- Local saved-scan retention now keeps up to 300 records and `/history` exports schema v2 dataset JSON with photo, stable image hash, media metadata, result, correction, notes, chat, OCR/model metadata, prompt versions, latency, and sync events.
...
- Latest captured configured-run Supabase request/error id on May 23, 2026: `019e5268-caca-7e0d-91f3-fcc91a57aefa`.
- Remote Supabase repair is blocked from this workspace until an authenticated Supabase CLI session, `SUPABASE_ACCESS_TOKEN`, or privileged Postgres connection is available; the inspected process environment has no Supabase access token, privileged Postgres connection, public Supabase Actions secrets, or `GEMINI_API_KEY` for local provider calls.
...
- Production readiness work is now tracked in standalone GitHub issues: #57, #58, #61, #62, #87, #88, #89, and #90 remain open.
...
| P0-003 | Medium | Backlog hygiene | Standalone production-readiness issues now exist, but they are not organized with release milestones or labels yet. | Live GitHub issue list on May 23, 2026 showed #57, #58, #61, #62, #87, #88, #89, and #90 open. | Add milestones/labels and keep issue comments tied to PR #103 evidence. |
...
| P1-001 | High | Scanner UX | Resolved on current branch: test mode bypasses camera-blocking copy and the denied-camera state still exposes fallback scan inputs. | `Scanner.test.tsx` covers blocked camera plus clean `/scan?test=1`; browser QA on May 22, 2026 passed `/scan?test=1` and denied-camera `/scan` at mobile viewport `390x844`. | Keep this in the browser smoke pass so test mode and camera denial do not regress. |
...
| P1-011 | High | Dataset quality | Cloud dataset persistence is still blocked even though local retention/export improved. | `MAX_SAVED_LOOKUPS = 300`, `DATASET_EXPORT_SCHEMA_VERSION = 2`, `getDatasetExport()`, and `/history` export button now preserve local dataset records with image hash/media metadata; `npm run verify:supabase` still fails. | Keep the local export as a fallback, but cloud dataset must preserve all synced scans with retention/export policy. |
...
| P1-013 | High | Visual search | Upload and pasted-image input exist and are live in the denied-camera fallback, but Lens-style mobile share-sheet input is not covered yet. | Scanner accepts JPEG/PNG/WebP upload and pasted image blobs; tests cover upload and pasted screenshots while camera is blocked; May 22 browser QA uploaded the engine fixture to a saved result screen, then hit provider-config `500` because the local server lacks `GEMINI_API_KEY`. | Rerun live upload with provider configuration, then add mobile share intent handling after the core camera and cloud gates are stable. |
