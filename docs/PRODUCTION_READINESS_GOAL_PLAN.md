...
| P1-005 | High | Alternatives | Candidate matches can now be selected as the corrected training label, but result ranking is still lightweight. | `candidateMatches[]` carries ranked part/confidence/reason, and `Mark correct` promotes an alternate into rating/correction/training status. There are still no per-candidate sources. | Add candidate source evidence and richer candidate-specific context. |
