...
| --- | --- | --- | --- |
| `/auth` | Signed out. Supabase config must be present for interactive sign-in. | Page renders without redirect loops. Email code and password methods are visible. Google and GitHub sign-in appear when Supabase exposes those providers. No local continue or fixture login control is present. | Snapshot shows `Sign in`, email field, code/password tabs, optional Google/GitHub OAuth buttons, and no local bypass. Console has no errors. |
