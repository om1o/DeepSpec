...
| --- | --- | --- | --- |
| `/auth` | Signed out. Supabase config must be present for interactive sign-in. | Page renders without redirect loops. Email code form is visible. Google and GitHub sign-in appear only when enabled. No local continue or fixture login control is present. | Snapshot shows `Sign in with a code`, email field, optional Google/GitHub OAuth buttons, and no local bypass. Console has no errors. |
