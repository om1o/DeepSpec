...
3. `npm run eval:identify:public` is the public-launch confidence gate. It builds a deterministic 300-sample set from the verified DrBimmer Hugging Face tree, balanced across 150 damage images and 150 part images, then verifies that at least 300 samples were attempted and passed with launch metrics present.
...
6. Server-side provider failover is configurable without code changes: `GEMINI_MODEL` sets the primary identify model, `GEMINI_FALLBACK_MODELS` adds comma-separated identify fallback models before the built-in fallback, and `GEMINI_CHAT_FALLBACK_MODELS` can override chat-specific fallbacks.
7. Eval summaries must include launch metrics: accuracy, invalid response rate, provider failure rate, retry count/rate, latency distribution, OCR usage count/rate, and rule-derived safety false positives/false negatives.
8. Model-quality failures such as `invalid_response`, wrong results, or vague results are still written to `.deepspec-eval/identify-failures.jsonl` in the saved-scan review shape.
9. Scanner production readiness requires the QA fixture, the beta eval, the public-launch eval, and the cloud verifier to pass without provider or Supabase availability blockers.
