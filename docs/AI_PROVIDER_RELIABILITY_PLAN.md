...
2. `npm run eval:identify:release` is the live provider release check. It uses the fixed 50-case DrBimmer release set, throttles provider calls, retries rate limits with long backoff, and stops after the configured provider availability failure count.
