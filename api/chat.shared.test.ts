...
describe("createChatResponse", () => {
  ...
  it("tries configured chat fallback models before the built-in fallback", async () => {
    const fetchSpy = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ error: { message: "quota exhausted" } }), {
      ...
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({
            candidates: [{ content: { parts: [{ text: "Use a second angle before buying parts." }] } }],
          }),
          {
            status: 200,
            headers: { "Content-Type": "application/json" },
    ...
    await expect(
      createChatResponse(
        { userMessage: "What should I check?" },
        {
          ...
          GEMINI_CHAT_FALLBACK_MODELS: " gemini-2.0-flash-lite , gemini-flash-lite-latest ",
        },
      ),
    ).resolves.toMatchObject({
      status: 200,
      body: {
        message: "Use a second angle before buying parts.",
        modelRun: {
          kind: "chat",
